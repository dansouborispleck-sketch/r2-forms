...
function Row({ analysis, onRedeploy, busy }) {
  ...
  const date = new Date(analysis.created_at).toLocaleDateString(lang === 'fr' ? 'fr-FR' : 'en-US', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });
  ...
  return (
    <div style={{ padding: 14, border: '1px solid #f0f0f0', borderRadius: 12, marginBottom: 12 }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: '#1A1A1A' }}>
        {analysis.titre || t('Questionnaire', 'Questionnaire')}
        {langueLabel && <span style={{ fontWeight: 400, color: '#aaa' }}> · {langueLabel}</span>}
      </div>
      <div style={{ fontSize: 11, color: '#aaa', marginBottom: 8 }}>
        {date}{analysis.nb_questions ? ` · ${analysis.nb_questions} ${t('questions', 'questions')}` : ''}
      </div>
      {deps.length === 0 && <div style={{ fontSize: 12, color: '#aaa', padding: '4px 0' }}>{t('Pas encore déployé', 'Not deployed yet')}</div>}
      {deps.map((d) => (
        <div key={d.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0' }}>
          ...
          {d.outil === 'excel'
            ? (hasXlsform && <button onClick={() => onRedeploy(analysis, 'excel', '')} style={{ background: '#1A1A1A', color: '#fff', border: 'none', borderRadius: 6, padding: '4px 10px', fontSize: 11, cursor: 'pointer' }}>{t('Retélécharger', 'Re-download')}</button>)
            : (d.form_url && <a href={d.form_url} target="_blank" rel="noopener noreferrer" style={{ background: '#1A1A1A', color: '#fff', border: 'none', borderRadius: 6, padding: '4px 10px', fontSize: 11, textDecoration: 'none' }}>{t('Accéder à', 'Open')}</a>)}
        </div>
      ))}
      {hasXlsform ? (
        <div style={{ display: 'flex', gap: 6, marginTop: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <select value={targetTool} onChange={(e) => setTargetTool(e.target.value)} style={{ flex: 1, minWidth: 110, padding: 6, border: '1px solid #E8E8E8', borderRadius: 6, fontSize: 12 }}>
            {REDEPLOY_TOOLS.map((tid) => (
              <option key={tid} value={tid}>{toolById(tid).name}{deployedTools.includes(tid) ? ' ' + t('(déjà déployé)', '(already deployed)') : ''}</option>
            ))}
          </select>
          <select value={targetLangCode} onChange={(e) => setTargetLangCode(e.target.value)} style={{ flex: 1, minWidth: 110, padding: 6, border: '1px solid #E8E8E8', borderRadius: 6, fontSize: 12 }}>
            <option value="">{t('Même langue', 'Same language')}{langueLabel ? ` (${langueLabel})` : ''}</option>
          ...
          >
            {busy ? '...' : t('Redéployer', 'Redeploy')}
          </button>
        </div>
      ) : (
        <div style={{ fontSize: 11, color: '#aaa', marginTop: 8 }}>{t('Redéploiement indisponible pour cette ancienne analyse.', 'Redeployment unavailable for this older analysis.')}</div>
      )}
    </div>
  );
}
    ...
    <div style={{ display: 'flex', position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,.5)', zIndex: 9999, alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '90%', maxWidth: 640, maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <div style={{ fontSize: 18, fontWeight: 700 }}>🚀 {t('Mes déploiements', 'My deployments')}</div>
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          {analyses === null && <div style={{ textAlign: 'center', color: '#aaa', padding: 20 }}>{t('Chargement...', 'Loading...')}</div>}
          {analyses && analyses.length === 0 && (
            <div style={{ textAlign: 'center', color: '#aaa', padding: 20 }}>{!user ? t('Non connecté', 'Not signed in') : t('Aucun questionnaire analysé pour le moment', 'No questionnaire analysed yet')}</div>
          )}
