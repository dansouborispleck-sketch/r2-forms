...
export function startGoogleAuth({ selectedTool, fileContent, pasteContent }) {
...
// selectedTool, content } si on revient d'une redirection OAuth Google reussie, sinon
// null. Nettoie le hash d'URL et le sessionStorage dans tous les cas ou un retour est
// detecte, pour ne pas re-consommer le meme token au prochain rendu/refresh.
export function consumeGoogleOAuthReturn() {
  ...
  return { googleAccessToken: token, selectedTool, content };
}
